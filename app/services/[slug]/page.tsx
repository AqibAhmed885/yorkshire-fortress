...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main><SiteHeader />
    ...
    <section className="yfs-service-content" id="main"><aside><div className="yfs-service-icon-large"><ServiceIcon name={service.icon} size={38} /></div><h3>At a glance</h3>{service.features.map((item) => <span key={item}><Check />{item}</span>)}<a href="/contact"><PhoneCall /> Request a callback</a></aside><article><p className="yfs-kicker">About this service</p><p className="lead">{service.intro}</p>{service.sections.map((section) => <section key={section.title}><h2>{section.title}</h2><p>{section.copy}</p></section>)}</article></section>
    <section className="yfs-service-fit"><div><Building2 /><p className="yfs-kicker">Where it fits</p><h2>Designed for real operating environments.</h2><ul>{service.suitableFor.map((item) => <li key={item}><Check />{item}</li>)}</ul></div><div><ClipboardCheck /><p className="yfs-kicker tan">What you receive</p><h2>A defined service, not a vague promise.</h2><ul>{service.clientReceives.map((item) => <li key={item}><Check />{item}</li>)}</ul></div></section>
