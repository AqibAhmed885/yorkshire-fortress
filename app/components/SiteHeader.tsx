...
export function SiteHeader({ solid = false }: { solid?: boolean }) {
  ...
  return (
    <header className={`${headerPosition} text-white`}>
      ...
      {open && (
        <div className={`absolute ${mobileTop} inset-x-0 max-h-[calc(100vh_-_78px)] overflow-y-auto border-t border-white/15 bg-navy px-6 py-4 shadow-2xl lg:hidden`}>
          <a className="flex items-center justify-between border-b border-white/15 py-4 font-bold" href="/about">About us<ArrowRight size={18} /></a>
          <button className="flex w-full items-center justify-between border-b border-white/15 bg-transparent py-4 text-left font-bold" aria-expanded={servicesOpen} onClick={() => setServicesOpen(!servicesOpen)}>Services<ChevronDown className={`transition-transform ${servicesOpen ? "rotate-180" : ""}`} size={18} /></button>
          {servicesOpen && <div className="bg-navy-deep px-4">{services.map((service) => <a className="flex items-center justify-between border-b border-white/10 py-3.5 text-sm text-[#dce2e8]" key={service.slug} href={`/services/${service.slug}`}>{service.shortTitle}<ArrowRight size={16} /></a>)}</div>}
          {links.map((link) => <a className="flex items-center justify-between border-b border-white/15 py-4 font-bold" key={link.href} href={link.href}>{link.label}<ArrowRight size={18} /></a>)}
        </div>
      )}
    </header>
  );
