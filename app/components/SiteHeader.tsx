    ...
    <header className={`${headerPosition} text-white`}>
      ...
      >
        ...
        <div className="flex items-center gap-[clamp(20px,2.2vw,38px)] text-sm font-semibold max-lg:hidden">
          <a className="transition-colors  hover:text-tan" href="/about">
            About us
          </a>
          <div
            className="relative"
          ...
          >
            <button
              className="flex items-center gap-1.5 bg-transparent py-8 font-extrabold transition-colors hover:text-tan"
              aria-haspopup="true"
                ...
                <div className="grid grid-cols-2 gap-px bg-line">
                  {services.map((service, index) => (
                    <a
                      ...
                      key={service.slug}
                    >
                      <span className="text-[10px] tracking-[.12em] text-muted">0{index + 1}</span>
                      <strong className="text-[14px] text-navy">{service.shortTitle}</strong>
                      <ArrowUpRight
          ...
          {links.map((link) => (
            <a className="transition-colors hover:text-tan" key={link.href} href={link.href}>
              {link.label}
            </a>
          ))}
          <a
